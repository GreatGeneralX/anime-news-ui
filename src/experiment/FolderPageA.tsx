// src/experiment/FolderPageA.tsx
...
export default function FolderPageA() {
  const { id } = useParams();
  ...
  const handleRemoveFromFolder = (articleId: number) => {
    ...
    // A -> B に切り替え
    navigate(`/experiment/folder/${folderId}/b`, { replace: true });
  };
  ...
  return (
    <div className="mt-16 sm:mt-0 px-4">
      ...
      <h1 className="text-2xl font-bold mb-1">[A] {folder.title || '未命名フォルダー'}</h1>
